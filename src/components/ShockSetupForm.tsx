...
export const ShockSetupForm = () => {
  ...
  const handleDownloadPDF = () => {
    ...
    // Add title
    doc.setFontSize(20);
    doc.text("Fox Float X2 Shock Settings - Trek EXE", 20, 20);
    ...
    doc.text(`Travel: ${travel}mm`, 20, 80);
    doc.text(`Preferred Feel: ${preferredFeel}`, 20, 90);
    doc.text(`Trail Condition: ${trailCondition}`, 20, 100);
    doc.text(`Priority: ${priority}`, 20, 110);
    
    // Add shock settings
    doc.setFontSize(16);
    doc.text("Recommended Settings:", 20, 130);
    
    doc.setFontSize(12);
    doc.text(`Air Pressure: ${settings.airPressure} PSI`, 20, 150);
    doc.text(`High-Speed Rebound (HSR): ${settings.hsr} clicks`, 20, 160);
    doc.text(`Low-Speed Rebound (LSR): ${settings.lsr} clicks`, 20, 170);
    doc.text(`High-Speed Compression (HSC): ${settings.hsc} clicks`, 20, 180);
    doc.text(`Low-Speed Compression (LSC): ${settings.lsc} clicks`, 20, 190);
    
    // Add footer
    doc.setFontSize(10);
    doc.text("Generated by Trek EXE Fox Float X2 Setup Guide", 20, 280);
    
    // Save the PDF
    doc.save("trek-exe-shock-settings.pdf");
  ...
  return (
    <Card className="w-full max-w-2xl mx-auto">
      <CardHeader>
        <CardTitle>Trek EXE Shock Setup Calculator</CardTitle>
      </CardHeader>
